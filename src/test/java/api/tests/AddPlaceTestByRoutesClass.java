...
public class AddPlaceTestByRoutesClass {
   Faker faker;
   AddPlaces addpla;
   ...
   @Test(priority = 2)
	public void getResponseByPlaceId() {
	   ...
	   logger.info("**************GET getting addPlace info*************");
		...
		Response response= UserEndPoints.getPlace(placeId);
   ...
   @Test(priority = 3)
	public void updateByPlaceId() {
     ...
     // Send PUT request to update the place with the new address
        Response responsed = UserEndPoints.updatePlace(placeId, newAddress);
     ...
     // Check the updated response by using the GET method again
        Response updatedresponse = UserEndPoints.getPlace(placeId);
